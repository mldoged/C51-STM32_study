...
void AD_Init(void)
{
	...
	DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_HalfWord;//不报错
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;//传输方向
	DMA_InitStructure.DMA_BufferSize = 4;//传输计数器大小
	DMA_InitStructure.DMA_Mode = DMA_Mode_Circular;//是否使用自动重装
	DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;//选择触发方式
...
//	
//	
//	while(DMA_GetFlagStatus(DMA1_FLAG_TC1) == RESET);//等待转换与转运完成
